...
def multiple_wake_model_ij_any_direction(v_i, r_0, Ct, x_ij, z, z_0, d_ij,theta, turbine_is_above):
    """
    Calculate the effective wind speed at turbine j due to the wake from turbine i,
    and return intermediate parameters

    Added for any direction (added angle (theta) between primary wind direction and given wind direction)
    """
    # Initialize shadowing flags
    partial_shadowing = False
    complete_shadowing = False
    ...
    #Change all instances of xij and dij to xij_prime and dij_prime to account for the angle theta

    theta_radians = theta * (pi / 180)  # Convert angle to radians

    if turbine_is_above == True:
        x_ij_prime = x_ij * np.cos(theta_radians) - d_ij * np.sin(theta_radians)
        d_ij_prime = x_ij * np.sin(theta_radians) + d_ij * np.cos(theta_radians)
    else:
        x_ij_prime = x_ij * np.cos(theta_radians) + d_ij * np.sin(theta_radians)
        d_ij_prime = -x_ij * np.sin(theta_radians) + d_ij * np.cos(theta_radians)
    ...
    # Calculate the wake radius at downstream distance x_ij
    r_i_xij = r_0 + alpha * x_ij_prime  # Wake radius at distance x_ij
    #print(f"r_i_xij (wake radius): {r_i_xij}")
    ...
    # Case 3: Partial shadowing (partial overlap)
        ...
        print("Case: Partial shadowing")
        # Calculate the height of the shadowed area (z_ij)
        term_1 = 4 * d_ij_prime**2 * r_i_xij**2
        term_2 = (d_ij_prime**2 - r_0**2 + r_i_xij**2) ** 2
        z_ij = (1 / d_ij_prime) * np.sqrt(term_1 - term_2) if term_1 > term_2 else 0
        #print(f"z_ij (height of shadowed area): {z_ij}")

        # Calculate the perpendicular distance L_ij
        L_ij = abs(d_ij_prime - np.sqrt(r_0**2 - (z_ij / 2) ** 2)) if z_ij > 0 else 0
        #print(f"L_ij (perpendicular distance): {L_ij}")
        ...
        if L_ij > 0:
            try:
                A_shadow_i = (
                    r_0**2 * np.arccos((d_ij_prime**2 + r_0**2 - r_i_xij**2) / (2 * d_ij_prime * r_0))
                    + r_i_xij**2 * np.arccos((d_ij**2 + r_i_xij**2 - r_0**2) / (2 * d_ij_prime * r_i_xij))
                    - 0.5 * d_ij_prime * z_ij
                )
                partial_shadowing = True
                print(f"A_shadow_i/A_0 (normalised shadowed area): {A_shadow_i/A_0}")
...
def mwm_identical_turbines_WD_any_direction(Ct, r_0, z, z_0, turbine_positions, theta):
    """
    Multiple Wake Model with Weibull Distribution Scaling.
    ...
    for j, (j_id, (x_j, y_j)) in enumerate(sorted_turbines):
        ...
        for i, (i_id, (x_i, y_i)) in enumerate(sorted_turbines[:j]):  # Only upstream turbines
            ...
            if x_ij > 0:
                ...
                # Determine if turbine j is above or below turbine i
                if y_j > y_i:
                    is_above = True
                else:
                    is_above = False
                ...
                # Calculate wake effect
                v_j_xij, _, _, _, _, _, _ = multiple_wake_model_ij_any_direction(v_i, r_0, Ct, x_ij, z, z_0, d_ij, theta, is_above)
                wake_deficit = (v_i - v_j_xij) / v_0  # Normalized wake deficit
                total_deficit_squared += wake_deficit**2  # Quadratic summation
...
def mwm_identical_turbines_BEM_any_direction(file_path, sheet_name, Ct, r_0, z, z_0, turbine_positions, theta):
    ...
    # Calculate Weibull parameters for each turbine using the wake model
    weibull_results = mwm_identical_turbines_WD_any_direction(Ct, r_0, z, z_0, turbine_positions, theta)

...
def mwm_identical_turbines_BEM_any_direction(file_path, sheet_name, Ct, r_0, z, z_0, turbine_positions, theta):
    ...
    Ensures Excel runs in the background efficiently using xlwings.
    ...
    - z_0 (float): Surface roughness length (m).
    - turbine_positions (dict): Dictionary with turbine IDs and (x, y) positions.
    - theta (float): Wind direction angle (degrees).
    ...
    # **Ensure Excel runs in the background efficiently**
    app = xw.App(visible=False, add_book=False)  # Prevents Excel UI pop-ups
...
def LCOE(file_path, sheet_name, Ct, r_0, z, z_0, turbine_positions, wind_rose, primary_direction=240):
    ...
    running Excel in the background via xlwings to reduce UI lag.

    Parameters:
    - file_path (str): Path to the BEM spreadsheet.
    - sheet_name (str): Sheet name in Excel for calculations.
    - Ct, r_0, z, z_0: Wind turbine parameters.
    - turbine_positions (dict): Dictionary of turbine positions.
    - wind_rose (dict): Wind direction frequencies.
    - primary_direction (int): Primary wind direction (default=240°).

    Returns:
    - float: Computed LCOE (£/MWhr).
    """
    # Start Excel in background mode (persistent session)
    ...
    try:
        ...
        # Compute energy yield while keeping Excel open
        ...
        total_energy_yield *= 1000  # Convert GWhr to MWhr

        # Compute LCOE
        num_turbines = len(turbine_positions)
        CAPEX_annual = (1_500_000 + 1_200_000 * 25.074) * num_turbines  # CAPEX
        OPEX_annual = 200_000 * num_turbines  # OPEX
        ...
        print(f"Total energy yield: {total_energy_yield:.2f} MWhr")
        print(f"LCOE: {LCOE_value:.2f} £/MWhr")

        return LCOE_value

    finally:
        # Ensure Excel is properly closed after computation
        wb.close()
        app.quit()
...
def next_generation_with_mutation(population, file_path, sheet_name, Ct, r_0, z, z_0, wind_rose, boundary, min_spacing):
    """
    ...
    - list: New population of 10 wind farm layouts.
    """
    # Compute fitness scores (LCOE)
    fitness_scores = {tuple(layout.items()): LCOE(file_path, sheet_name, Ct, r_0, z, z_0, layout, wind_rose) for layout in population}
    
    # Select top 3 fittest layouts
    sorted_population = sorted(fitness_scores, key=fitness_scores.get)
    parent_1 = dict(sorted_population[0])  # Most fit
    parent_2 = dict(sorted_population[1])  # Second most fit
    parent_3 = dict(sorted_population[2])  # Third most fit
    ...
    # Apply mutation to 8 offspring (excluding top 2 parents)
...
def compute_LCOE_parallel(args):
    """ Wrapper for multiprocessing to compute LCOE in parallel. """
    layout, file_path, sheet_name, Ct, r_0, z, z_0, wind_rose = args
    return layout, LCOE(file_path, sheet_name, Ct, r_0, z, z_0, layout, wind_rose)
...
def genetic_algorithm(file_path, sheet_name, boundary, min_spacing, num_turbines_range, 
    ...
    - sheet_name (str): Name of the sheet to interact with.
    - boundary (numpy array): UTM boundary coordinates.
    - min_spacing (float): Minimum spacing between turbines (3D rule).
    - num_turbines_range (tuple): Min and max number of turbines allowed.
    - Ct (float): Thrust coefficient.
    - r_0 (float): Rotor radius (m).
    - z (float): Hub height (m).
    ...
    - convergence_threshold (float): Stop if LCOE improvement over 5 generations is less than this fraction.

    Returns:
    - list: Top 5 best wind farm layouts throughout the process.
    - dict: Best wind farm layout from the final generation.
    - float: Best LCOE from the final generation.
    """
    ...
    # Track last 5 generations' best LCOEs for convergence checking
    recent_LCOEs = []
    ...
    for generation in range(1, max_generations + 1):
        ...
        # **Step 3: Generate Next Generation with Mutation**
        new_population = next_generation_with_mutation(population, file_path, sheet_name, Ct, r_0, z, z_0, wind_rose, boundary, min_spacing)
        ...
        # **Step 4: Evaluate Fitness of New Population (with caching)**
        print("Evaluating fitness (LCOE) for each layout...")
        fitness_scores = {}
        ...
        # **Step 5: Select the Fittest Individual**
        ...
        print(f"⏳ Generation {generation} completed in {time.time() - start_time:.2f} seconds.")
    ...
    return best_layouts_sorted, final_best_layout, final_best_LCOE
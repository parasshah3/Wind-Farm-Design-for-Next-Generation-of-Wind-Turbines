...
def LCOE(file_path, sheet_name, Ct, r_0, z, z_0, turbine_positions, wind_rose, primary_direction=240):
    ...
    # Constants
    CAPEX_PER_TURBINE = 1_500_000  # 23M per turbine
    TURBINE_COST_PER_MW = 1_200_000  # £1.2M per MW per turbine
    OPEX_PER_TURBINE = 200_000  # £200k per turbine per year
    ...
    print(f"LCOE for current individual: {LCOE_value:.2f} £/MWhr")
...
def genetic_algorithm(file_path, sheet_name, boundary, min_spacing, num_turbines_range, 
    ...
    for generation in range(1, max_generations + 1):
        ...
        print(f"The largest number of turbines in new generation: {max(len(layout) for layout in new_population)}")
        print(f"The lowest number turbines in new generation: {min(len(layout) for layout in new_population)}")

        # Step 4: Evaluate Fitness of New Population
        fitness_scores = {tuple(layout.items()): LCOE(file_path, sheet_name, Ct, r_0, z, z_0, layout, wind_rose) 
                          for layout in new_population}

        # Identify the fittest layout of this generation
        best_layout = min(fitness_scores, key=fitness_scores.get)
        best_LCOE = fitness_scores[best_layout]

        # Store best layout and LCOE
        best_layouts.append((dict(best_layout), best_LCOE, generation))
        best_LCOEs.append(best_LCOE)

        # Print best layout and LCOE
        print(f"Best LCOE this generation: {best_LCOE:.2f} £/MWhr, with number of turbines: {len(best_layout)}")

...
def calculate_average_turbine_spacing(layout_dict, k=4):
    ...
    if k >= n:
        print(f"Number of turbines ({n}) is less than k ({k}).")
        print("***-----***-----***-----***-----***-----***-----***-----***-----***-----***")
        print("Settting arbitrary values for spacing metrics.")
        avg_spacing = 5*2*124.7
        min_spacing = 124.7*2*3
        max_spacing = 8*2*124.7
    ...
    try:
        ...
        for layout in population:
            ...
            if layout_tuple in fitness_scores:
                print(f"Using cached LCOE: {fitness_scores[layout_tuple]:.6f} for layout.")
                print("")
            else:
        ...
        for generation in range(1, max_generations + 1):
            ...
            for layout in new_population:
                ...
                if layout_tuple in lcoe_cache:
                    ...
                    print(f"Turbine layout already evaluated. Using cached LCOE: {fitness_scores[layout_tuple]:.6f}")
                else:
                    lcoe_value = LCOE(file_path, sheet_name, Ct, r_0, z, z_0, layout, wind_rose)
            ...
            print(f"Best LCOE this generation: {best_LCOE:.6f} £/MWhr, with number of turbines: {len(best_layout_tuple)}")
            print(f"Best Layout: {dict(best_layout_tuple)}")

            # **Step 8: Convergence Check**
            recent_LCOEs.append(best_LCOE)
            if len(recent_LCOEs) > 10:
            ...
            if len(recent_LCOEs) == 10:
                ...
                if LCOE_change < convergence_threshold:
                    print(f"✅ Convergence reached: LCOE change < {convergence_threshold * 100:.4f}% over last 10 generations.")
                    break
...
def define_parameter_space(rotor_diameter, A, B, alpha_deg, 
    ...
    """
    theta_tolerance = 8  # Tolerance for boundary angle
    # Define limits
    D = rotor_diameter
    ...
    theta_vals = np.linspace(126.8, 127.8, theta_steps)
...
def evaluate_all_arrangements(file_path, sheet_name, boundary, 
                      Ct, r_0, z, z_0, wind_rose, A, B, alpha_deg,a_steps, b_steps, theta_steps):

    # Open Excel Session Once**
    app = xw.App(visible=False, add_book=False)
    app.display_alerts = False  
    app.screen_updating = False  
    wb = xw.Book(file_path)
    ...
    # Initialise population (define searh space)
    param_space = define_parameter_space(r_0*2, A, B, alpha_deg, a_steps, b_steps, theta_steps)

    all_layouts_and_parameters = [] # Store all generated layouts, a, b, and theta_deg as a list of dictionaries

    # Generate all layouts
    for a, b, theta_deg in param_space:
        generated_layout = generate_regular_arrangement(a, b, theta_deg, A, B, alpha_deg)
        all_layouts_and_parameters.append((generated_layout, a, b, theta_deg))

    print(f"Generated all layouts,  {len(all_layouts_and_parameters)} in total")
    print("------------------------------------------------------------")
    print(" ")

    # Evaluate LCOE of all layouts
    results = [] # Store all results as a list of tuples (layout, LCOE)

    for idx, (layout, a, b, theta_deg) in enumerate(all_layouts_and_parameters):

        print(f"Evaluating layout {idx + 1} of {len(all_layouts_and_parameters)} with a: {a}metres (a/A = {a/A:.4f}), b: {b}metres (b/B = {b/B:.4f}), theta: {theta_deg} degrees")
        
       
        LCOE_value = LCOE(file_path, sheet_name, Ct, r_0, z, z_0, layout, wind_rose)
        results.append((layout, LCOE_value, a, b, theta_deg))    
    
    # Sort by LCOE (asecending order)
    sorted_results = sorted(results, key=lambda x: x[1])

    # Extract best layout and LCOE
    best_layout = sorted_results[0][0]
    best_LCOE = sorted_results[0][1]
    best_a = sorted_results[0][2]
    best_b = sorted_results[0][3]
    best_theta_deg = sorted_results[0][4]
    ...
    return sorted_results, best_layout, best_LCOE, best_a, best_b, best_theta_deg
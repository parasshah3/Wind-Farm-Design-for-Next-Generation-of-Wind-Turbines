...
def define_parameter_space(rotor_diameter, A, B, alpha_deg, 
    ...
    theta_vals = np.linspace(alpha_deg - 2, alpha_deg - theta_tolerance, theta_steps)
...
def evaluate_all_arrangements(file_path, sheet_name, boundary, 
    ...
    for idx, (layout, a, b, theta_deg) in enumerate(all_layouts_and_parameters):
        ...
        print(f"Evaluating layout {idx + 1} of {len(all_layouts_and_parameters)} with a: {a}metres (a/A = {a/A:.2f}), b: {b}metres (b/B = {b/B:.2f}), theta: {theta_deg} degrees")

...
# 🔹 **NEW FUNCTION: Calculate Weibull parameters from mean wind speed and standard deviation**
def calculate_weibull_params_vj(U_mean, sigma):
    """
    Calculate Weibull shape (k) and scale (λ) factors based on given mean wind speed and standard deviation.

    Parameters:
    U_mean (float): Mean wind speed (m/s)
    sigma (float): Standard deviation of wind speed (m/s)

    Returns:
    k (float): Weibull shape factor
    lambda_ (float): Weibull scale factor
    """
    if U_mean <= 0 or sigma <= 0:
        raise ValueError("Mean wind speed and standard deviation must be positive.")

    # Solve for Weibull shape factor k
    k = brentq(equation, 0.5, 10, args=(sigma, U_mean))

    # Compute Weibull scale factor λ
    lambda_ = U_mean / special.gamma(1 + 1 / k)

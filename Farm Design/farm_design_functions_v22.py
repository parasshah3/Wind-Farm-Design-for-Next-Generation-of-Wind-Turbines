...
def generate_random_layout(boundary, min_spacing, num_turbines_range):
    ...
    print(f"Randomly generated turbines: {turbines}")
    return turbines
...
def LCOE(file_path, sheet_name, Ct, r_0, z, z_0, turbine_positions, wind_rose, primary_direction=240):
    """
    Compute the 15 year lifetime Levelized Cost of Energy (LCOE) for a given wind turbine arrangement.
    ...
    # Constants
    CAPEX_PER_TURBINE = 91_000_000  
    TURBINE_COST_PER_MW = 0  # £1.2M per MW per turbine
    OPEX_PER_TURBINE = 2_613_000  # £400k per turbine per year
    INITIAL_COST = 450_000_000  # £50M initial cost
    TURBINE_CAPACITY_MW = 25.157
    LIFETIME = 50  # 45 year lifetime  
    ...
    print(f"Total energy yield for current individual: {total_energy_yield:.2f} MWhr")
 

    # Compute total lifetime costs
    CAPEX_annual = (CAPEX_PER_TURBINE + TURBINE_COST_PER_MW * TURBINE_CAPACITY_MW) * num_turbines + INITIAL_COST
    OPEX_LIFETIME = OPEX_PER_TURBINE * num_turbines * LIFETIME
    total_lifetime_cost = CAPEX_annual + OPEX_LIFETIME

    # Compute LCOE (fitness function)
    LCOE_value = total_lifetime_cost / (total_energy_yield * LIFETIME) if total_energy_yield > 0 else float('inf')
    print(f"LCOE for current individual: {LCOE_value:.4f} £/MWhr")
    print(" ")
    ...
    try:
        ...
        for layout in population:
            ...
            if layout_tuple in fitness_scores:
                print(f"Using cached LCOE: {fitness_scores[layout_tuple]:.2f} for layout.")
            else:

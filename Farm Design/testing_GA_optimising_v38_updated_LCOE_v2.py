...
from farm_design_functions_v28 import genetic_algorithm
...
num_turbines_range = (17, 22)  # Min & max turbines

# Run the Genetic Algorithm
best_layouts, final_best_layout, final_best_LCOE,lowest_LCOE_per_gen, second_lowest_LCOE_per_gen = genetic_algorithm(
    file_path, sheet_name, boundary_utm, min_spacing, num_turbines_range,
    Ct, r_0, z, z_0, wind_rose, max_generations=250, convergence_threshold=0.00001
)

# Print top 5 layouts
print("\nTop 5 Optimal Wind Farm Layouts:")
for i, (layout, lcoe, gen) in enumerate(best_layouts, 1):
    print(f"Rank {i}: LCOE = {lcoe:.6f} £/MWhr (Generation {gen}) | {len(layout)} turbines")

# Print final best layout details
print("\nFinal Best Wind Farm Layout:")
print(f"LCOE: {final_best_LCOE:.6f} £/MWhr")
...
# LCOE vs Generation Plot (Formatted)
fig, ax = plt.subplots(figsize=(8, 5))
ax.plot(range(1, len(lowest_LCOE_per_gen)+1), lowest_LCOE_per_gen, 
        marker='o', label="Fittest", linewidth=1.5)
ax.plot(range(1, len(second_lowest_LCOE_per_gen)+1), second_lowest_LCOE_per_gen, 
        marker='o', color='orange', label="2nd Fittest", linewidth=1.5)

# Axes labels and title
ax.set_xlabel("Generation")
ax.set_ylabel(r"LCOE [£/MWh]")
ax.set_title("Fittest and 2nd Fittest LCOE vs Generation")

# Grid and formatting
ax.grid(True, linestyle='-', linewidth=0.7, alpha=0.7)
ax.ticklabel_format(style='plain', axis='both')  # Removes scientific notation
ax.tick_params(axis='both', labelsize=12)        # Set tick label size
